...
__device__ __host__ su2Element::su2Element() {
  element[0] = 1;
  element[1] = 0;
  element[2] = 0;
  element[3] = 0;
...
__device__ __host__ su2Element::su2Element(double *input) {
  for (int i = 0; i < 4; i++) {
    element[i] = input[i];
  }
}
...
__device__ __host__ double su2Element::trace() { return 2 * element[0]; }

__device__ __host__ su2Element operator*(const su2Element &e1, const su2Element &e2) {
  double product[4];
  product[0] =
...
__device__ __host__ su2Element su2Element::adjoint() {
  double adjoint[4] = {element[0], -element[1], -element[2], -element[3]};
  return su2Element(&adjoint[0]);
}

std::ostream &operator<<(std::ostream &os, const su2Element &e) {
  os << "(" << e[0] << "," << e[1] << ") ";
  os << "(" << e[2] << "," << e[3] << ") " << std::endl;
  os << "(" << -e[2] << "," << e[3] << ") ";
  os << "(" << e[0] << "," << -e[1] << ") " << std::endl;
...
__device__ __host__ double su2Element::operator[](int i) const { return element[i]; }
__device__ __host__ double &su2Element::operator[](int i) { return element[i]; }

su2Element su2Element::randomize(double delta,
...
__device__ su2Element su2Element::randomize(double delta,
                                             curandState_t &state) {
...
__device__ __host__ su2Element su2Element::randomize(double alpha, double *pnt) {
  double norm = 0;
  for (int i = 0; i < 3; i++) {
    norm += pnt[i] * pnt[i];
  }
...
__device__ __host__ void su2Element::renormalize() {
  double norm = 0;
  for (int i = 0; i < 4; i++) {
