...
__global__ void kernel_probe_site(su2Action<dim> act, su2Element *fields,
                                  curandState_t *randStates, int *hitCounts,
                                  int multiProbe, double delta, int odd,
                                  int mu) {
...
__global__ void kernel_initIteration(curandState *state, su2Element *fields,
                                     int *hitCounts, int nMax, int dim) {
...
cudaMetropolizer<dim>::cudaMetropolizer(su2Action<dim> iAction, int iMultiProbe,
  ...
  cudaMalloc(&randStates, sizeof(curandStateMtgp32) * randStateCount);
  cudaMalloc(&randStateParams, sizeof(mtgp32_kernel_params) * randStateCount);

  for (int i = 0; i < randStateCount; i++) {
    curandMakeMTGP32Constants(mtgp32dc_params_fast_11213, &randStateParams[i]);
    if (i = randStateCount - 1) {
      curandMakeMTGP32KernelState(&randStates[i], mtgp32dc_params_fast_11213,
                                  &randStateParams[i], blockCount % 200,
                                  42 + blockCount / 200);
    } else {
      curandMakeMTGP32KernelState(&randStates[i], mtgp32dc_params_fast_11213,
                                  &randStateParams[i], 200,
                                  42 + blockCount / 200);
    }
  }
